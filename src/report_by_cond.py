...
for experimental_condition in ["img", "sf", "toon"]:
    print(f"Plotting results for {experimental_condition}...")
    folder = os.path.join(folder, f"results_{experimental_condition}")
    ...
    res_df.groupby("model")["accuracy"].describe()
    ...
    for scorer in scorers:
        fig, ax = plt.subplots(figsize=(16*plot_sf, 9*plot_sf))
        sns.violinplot(x="model", y=scorer, hue="model", data=res_df, ax=ax, inner="stick", fill=False)
        # plt.show()
        plt.savefig(os.path.join(outf, f"model_dist_{scorer}.png"), dpi=300)
